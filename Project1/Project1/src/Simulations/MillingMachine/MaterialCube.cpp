...
void MaterialCube::createHeightMapQuad()
{
	...
	// create vao for ccw tesselation
	glBindVertexArray(VAO_quadCCW);
	glBindBuffer(GL_ARRAY_BUFFER, VBO);

	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, EBO_quadCCW);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, indsCCW.size() * sizeof(int), &indsCCW[0], GL_STATIC_DRAW);
...
{  
	...
	for (int j = 0; j < 4; j++) {   
		...
		for (int i = 0; i < divisionX / 100.0f; i++)
		{
			...
			glBindVertexArray((j==0 || j==3) ? VAO_quadCCW : VAO_quads);
			glPatchParameteri(GL_PATCH_VERTICES, 4);
			glDrawElements(GL_PATCHES, 4, GL_UNSIGNED_INT, 0);
		}
	}
	...
	divisionX(divisionX), divisionY(divisionY), modelMatrix(glm::identity<glm::fmat4x4>())
{
	...
	glGenVertexArrays(1, &VAO_quadCCW);
	glGenVertexArrays(1, &VAO_triangles);
	glGenBuffers(1, &VBO);
	glGenBuffers(1, &EBO_quad);
	glGenBuffers(1, &EBO_quadCCW);
